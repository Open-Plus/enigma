...
def FileEntryComponent(name, absolute = None, isDir = False):
	...
	if screenwidth and screenwidth == 1920:
		res = [(absolute, isDir), (eListboxPythonMultiContent.TYPE_TEXT, 90, 3, 770, 60, 1, RT_HALIGN_LEFT, name)]
	else:
		res = [(absolute, isDir), (eListboxPythonMultiContent.TYPE_TEXT, 35, 1, 770, 20, 0, RT_HALIGN_LEFT, name)]
	if isDir:
...
def MultiFileSelectEntryComponent(name, absolute = None, isDir = False, selected = False):
	screenwidth = getDesktop(0).size().width()
	if screenwidth and screenwidth == 1920:
		res = [(absolute, isDir, selected, name), (eListboxPythonMultiContent.TYPE_TEXT, 150, 3, 770, 60, 1, RT_HALIGN_LEFT, name)]
	else:
		res = [(absolute, isDir, selected, name), (eListboxPythonMultiContent.TYPE_TEXT, 55, 1, 770, 20, 0, RT_HALIGN_LEFT, name)]
	if isDir:
		png = LoadPixmap(cached=True, path=resolveFilename(SCOPE_ACTIVE_SKIN, "extensions/directory.png"))
	else:
		extension = name.split('.')
